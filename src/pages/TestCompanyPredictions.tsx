...
import { getCompanyPredictions, CompanyForecastPayload } from "@/lib/techpulse-api";
...
const TestCompanyPredictions = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const forecast = await getCompanyPredictions(payload);
      setResult(forecast);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-background p-8">
      <div className="max-w-2xl mx-auto space-y-6">
        <h1 className="text-3xl font-bold text-foreground">Test Company Predictions</h1>
        <p className="text-muted-foreground">
          Testa l'endpoint POST /company/predictions del backend FastAPI.
        </p>
        ...
        <Card>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="timeframe">Orizzonte temporale</Label>
                <Select 
                  value={timeframeMonths.toString()} 
                  onValueChange={(v) => setTimeframeMonths(Number(v))}
                >
                  <SelectTrigger>
        ...
        {result && (
          <Card className="border-primary">
            <CardHeader>
              <CardTitle>Previsione TechPulse</CardTitle>
            </CardHeader>
            <CardContent>
...
export default TestCompanyPredictions;
